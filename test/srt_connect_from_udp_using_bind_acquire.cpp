/***
 *  @name Test chế độ bind aquire trên SRT
 *  
 *  - cho trước một UDP socket đã trao đổi dữ liệu với nhau
...
int init_srt(srt_context_s *cxt)
{
    ...
    /**Create UDP socket*/
    int udp_socket = create_udp_socket(cxt);
    ...
    st = srt_bind_acquire(cxt->srt_socket, udp_socket);
    if (st == SRT_ERROR)
    {
