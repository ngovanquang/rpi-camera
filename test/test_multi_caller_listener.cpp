/***
 *  @name Test multi caller listener trên một udp socket, khi caller có streamid xác định sẽ truyền dữ liệu tương ứng.
 *
 *  Kịch bản: (VLC caller 1 - streamid01) <---- (UDP port máy nhận 1) ---- (UDP port 5000) ---- (SRT listener 1)
 *            (VLC caller 2 - streamid02) <---- (UDP port máy nhận 2) ---- (UDP port 5000) ---- (SRT listener 2)
...
int SrtTestListenCallback(void *opaq, SRTSOCKET ns, int hsversion, const struct sockaddr *peeraddr, const char *streamid)
{
    ...
    if (strcmp(cxt->stream_id, streamid) == 0)
    {
        return OK;
    }
    else
...
{
    srt_context_s *srt_cxt = (srt_context_s *)cxt;
    // send_mpegts_file2(srt_cxt->ts_file, srt_cxt);
    while (1)
    {
    ...
    char buffer[1316] = {0};
    FILE *fp = fopen(srt_cxt->ts_file, "rb");
    int srt_sock;
    int st;
    int ret;
    while (!feof(fp))
    {
        ...
        if (srt_cxt->srt_mode == SRT_MODE_LISTENER)
        {
            for (int i = 0; i < MAX_SRT_CLIENT; ++i)
            {
                if (srt_cxt->their_fd[i] != SRT_INVALID_SOCK)
                {
                    srt_sock = srt_cxt->their_fd[i];
                    st = srt_sendmsg2(srt_sock, buffer, ret, NULL);
                    if (st == SRT_ERROR)
                    {
                        LOG_ERROR("srt_sendmsg: %s, socket %d\n", srt_getlasterror_str(), srt_sock);
                        srt_cxt->their_fd[i] = SRT_INVALID_SOCK;
                    }
                }
            }
        }
...
int main(int argc, char const *argv[])
{
    ...
    if (argc != 5)
    {
        LOG_ERROR("Usage: %s <host> <port> <input_file1.ts> <input_file2.ts>\n", argv[0]);
        return FAILED;
    }
    ...
    srt_context_s *listener1 = allocate_srt_context();
    srt_context_s *listener2 = allocate_srt_context();

    listener1->ip_address = argv[1];
    listener1->ip_port = atoi(argv[2]);
    listener1->srt_mode = SRT_MODE_LISTENER;
    listener1->stream_id = "file01";
    listener1->udp_socket = udp_socket;
    listener1->ts_file = argv[3];

    listener2->ip_address = listener1->ip_address;
    listener2->ip_port = listener1->ip_port;
    listener2->srt_mode = SRT_MODE_LISTENER;
    listener2->stream_id = "file02";
    listener2->udp_socket = udp_socket;
    listener2->ts_file = argv[4];


    // init srt
    if (FAILED == init_srt(listener1))
    {
        return FAILED;
    }

    if (FAILED == init_srt(listener2))
    {
        return FAILED;
    }


    pthread_t send_file1_thread, send_file2_thread;

    st = pthread_create(&send_file1_thread, NULL, send_ts_handle, (void *)listener1);
    if (st)
    {
    ...
    st = pthread_create(&send_file2_thread, NULL, send_ts_handle, (void *)listener2);
    if (st)
    {
        LOG_ERROR("Error creating thread for receiver: %d\n", st);
        return FAILED;
    ...
    destroy_srt(listener1);
    destroy_srt(listener2);
    return 0;
}